...
PRIV_KEY = "2c303c5425d8660e16391595c5eaa7ee8227096b66572be41242ed8b142f6add"
PUB_KEY = "455d048c62c53c54f4a3d0e26e95bdac9f69f29343156b2a7a7dea7ae2be928abae2560337de4d637f85a3d458c39fe7cdf52232d07216d38e7a6ed27070f70e"

def tea_decipher(v, k):
    """单次 TEA 解密，输入 8 字节块 + 16 字节密钥，输出 8 字节明文。"""
    op = 0xffffffffL
...
def tea_decrypt(v, k):
    """按 8 字节分块对任意长度数据执行 TEA 解密，剩余尾部原样返回。"""
    num = len(v) / 8 * 8
    ret = ''
...
def IsGoodLogBuffer(_buffer, _offset, count):
    """递归检查缓冲区中是否存在 count 条合法的 xlog 记录，并返回校验结果。"""
    ...
    headerLen = 1 + 2 + 1 + 1 + 4 + crypt_key_len  # 头部固定字段长度 + 密钥长度
